#define sync_high                   44000
#define sync_low                    36000
#define one_high                    2200
#define one_low                     1800
#define zero_high                   1100
#define zero_low                    900

#define no_of_bits                  30
...
void decode_sharp_protocol(unsigned char *addr, unsigned char *cmd);
...
void main()
{
     ...
     while(1)
     {
             if(received)
             {
                 decode_sharp_protocol(&address, &command);

                 if((address <= 0x1F) && (command != 0xFF))
                 {
